...
After installation you will ned to create the default database. Go to https://localhost:8081/_explorer/index.html, select Explorer, Select "New Container". Name the database "nadobe-dev", and the container id "Accreditations". Supply "/Owner" (capital O, matching the `Owner` field's `DataMember` name on `Accreditation`) as partition key. 
